<script lang="ts" setup>
  ...
  const handleSizeChange1 = async (val: number) => {
      ...
      let deviceModel = selectedType.value;
      let deviceMac = selectedUID.value;
      let deviceSn = selectedChannel.value;
      let country = selectedsales.value;
    
      try {
        const response = await getEquipmentList(page_Id, page_Size, deviceModel, deviceMac, deviceSn, country);
        ...
        if (response.data.list !== null) {
          ...
          tableData1.value = responseData.map((item:  {
            ...
            const startAt = new Date(item.device_info.maintenance_date.start_at * 1000);
            const endAt = new Date(item.device_info.maintenance_date.end_at * 1000);
            ...
            if (days > 0) {
              maintenancePeriod = `${days}天`;
              if (hours > 0) maintenancePeriod += ` ${hours}小时`;
              if (minutes > 0) maintenancePeriod += ` ${minutes}分钟`;
            } else if (hours > 0) {
              maintenancePeriod = `${hours}小时`;
              if (minutes > 0) maintenancePeriod += ` ${minutes}分钟`;
            } else if (minutes > 0) {
              maintenancePeriod = `${minutes}分钟`;
            } else {
              maintenancePeriod = '0';
            }
             ...
             // 设置runningState值
             let runningState = '';
             switch(item.fault.alert_status) {
               case 0:
                 runningState = '未处理';
                 break;
               case 1:
               ...
               case 3:
                 ...
                 break;
               default:
                 runningState = '未知'; // 处理未知状态
             }
          
            return {
              waxMaterialType: item.device_info.model,
              deviceMAC: item.device_info.mac,
              deviceSN: item.device_info.sn,
              activationState: item.device_info.activation_status === 1 ? '已激活' : '未激活',
              activationTime: new Date(item.device_info.activation_at * 1000).toLocaleString(),
              activationPeriod: new Date(item.device_info.device_expire_at * 1000).toLocaleString(),
              maintenancePeriod,
              softwareRelease: item.device_info.software_version,
              softwareReleaseDate: item.device_info.software_release_date,
              ...
              salesArea: mapCountryToChinese(item.order.country),
              outboundOrderNumber: item.order.inventory_order_id,
              outboundTime: new Date(item.order.out_depot_at * 1000).toLocaleString(),
              // logisticsSigningTime: new Date(item.order.receive_at * 1000).toLocaleString(),
              deviceIP: item.transfer.ip,
              IPArea: item.transfer.ip_location,
              fleeGoods: item.transfer.alert ? '是' : '否',
              reportingTime: item.transfer.create_at,
              malfunctionAlarm: item.fault.alert ? '是' : '否',
              faultType: item.fault.type,
              faultReportingTime: item.fault.create_at,
              runningState
            }
          });
  ...
  const dynamicColumns1 = ref([
    // { prop: 'activationState', label: '激活/绑定状态', width: '235', sortable: false, filterable: true, filterOptions:[
    //     { label: '全部', value: 'all' },
    //     { label: '已激活', value: '已激活' },
    //     { label: '未激活', value: '未激活' }
    //   ] },
    { prop: 'activationState', label: '激活/绑定状态', width: '235', sortable: false },
    { prop: 'activationTime', label: '激活/解绑时间', sortable: false, width: '235' },
    { prop: 'activationPeriod', label: '激活有效期（天）', sortable: false, width: '235' },
    { prop: 'maintenancePeriod', label: '维保周期', sortable: false, width: '235' },
    { prop: 'softwareRelease', label: '软件版本', sortable: false, width: '235' },
    { prop: 'softwareReleaseDate', label: '软件发布日期', sortable: false, width: '235' },
    {
      label: '关联订单信息',
      columns: [
        { prop: 'salesOrderNumber', label: '销售订单号', width: '120', sortable: false },
        { prop: 'salesman', label: '销售员', width: '106', sortable: false },
        { prop: 'customerName', label: '客户名称(经销商/终端客户)', width: '200', sortable: false },
        { prop: 'orderAddress', label: '订单地址', width: '134', sortable: false },
        { prop: 'salesArea', label: '归属销售区域', width: '133', sortable: false },
        { prop: 'outboundOrderNumber', label: '出库订单号', width: '186', sortable: false },
        { prop: 'outboundTime', label: '出库时间', width: '134', sortable: false },
        // { prop: 'logisticsSigningTime', label: '物流签收时间', width: '133', sortable: false }
      ]
    },
    {
      label: '关联订单信息',
      columns: [
        { prop: 'deviceIP', label: '设备IP', width: '120', sortable: false },
        { prop: 'IPArea', label: 'IP所在区域', width: '106', sortable: false },
        { prop: 'fleeGoods', label: '是否窜货报警', width: '134', sortable: false },
        { prop: 'reportingTime', label: '上报时间', width: '134', sortable: false },
        // { prop: 'iPArea', label: '处理状态', width: '133', sortable: false }
      ]
    },
    {
      label: '关联故障报警信息',
      columns: [
        // { prop: 'malfunctionAlarm', label: '是否故障报警', width: '240', sortable: false , filterable: true, filterOptions: [
        //       { label: '全部', value: 'all' },
        //       { label: '是', value: '是' },
        //       { label: '否', value: '否' }
        //    ]},
        { prop: 'malfunctionAlarm', label: '是否故障报警', width: '240', sortable: false },
        // { prop: 'falseAlarm', label: '是否蜡材窜货报警', width: '240', sortable: false , filterable: true, filterOptions: [
        //       { label: '全部', value: 'all' },
        //       { label: '是', value: '是' },
        //       { label: '否', value: '否' }
        //    ]},
        { prop: 'faultType', label: '故障类型', sortable: false, width: '164' },
        { prop: 'faultReportingTime', label: '上报时间', sortable: false, width: '164' },
        { prop: 'runningState', label: '当前设备运行状态', sortable: false, width: '164' }
      ]
    }
  ])
  ...
  const allOptions = computed(() => {
    ...
    const { getStateStatistics, getEquipmentType, getEquipmentPerformance, getEquipmentList, getCountriesList } = useRegisterApi();
    ...
    const fetchEquipmentType = async () => {
      try {
        const response = await getStateStatistics();
        console.log('User account details fetched successfully:', response);
        console.log(response.data.list)
        ...
        const processAlertNumStats = () => {
          response.data.list.forEach((item: { status: any; num: string }) => {
            switch (item.status) {
              case 0:
    ...
    const fetchEquipmentType1 = async () => {
      try {
        ...
        equipmentOptions.value = uniqueItems.map((item: string) => ({
          value: item,
          label: item
        }));

        waxMaterialType.value = uniqueItems.map((item: string) => ({
          value: item,
          label: item
        })); 
    ...
    const statisticalTime = ref('')
    const avgCompletion = ref('0')

    const avgUtilization = ref('0')

    // const getFormattedAvgCompletion = (value) => {
    ...
    const fetchEquipmentPerformance = async () => {
      const deviceModel = equipmentType.value
      try {
        const response = await getEquipmentPerformance(deviceModel);
        console.log('User account details fetched successfully:', response);
        console.log(response.data)
        if (response.data !== null) {

          const convertDate = (date: { split: (arg0: string) => [any, any]; }) => {
            const [year, month] = date.split('-');
            return `${year}年${parseInt(month, 10)}月`;
          };
          
          const formattedDate = computed(() => convertDate(response.data.date));

          statisticalTime.value = formattedDate.value
          avgCompletion.value = response.data.avg_completion
          avgUtilization.value = response.data.avg_utilization

          const avgCompletionValue = parseFloat(avgCompletion.value);
          const avgUtilizationValue = parseFloat(avgUtilization.value);

          if (avgCompletionValue >= 0) {
            imgDownwardTriangle.value = '/upward_triangle.png'
          } else {
            imgDownwardTriangle.value = '/downward_triangle.png'
          }

          if (avgUtilizationValue >= 0) {
            imgUpwardTriangle.value = '/upward_triangle.png'
          } else {
            imgUpwardTriangle.value = '/downward_triangle.png'
          }
        }
        // const uniqueItems = Array.from(new Set(response.data.list)) as string[];
        // equipmentOptions.value = uniqueItems.map((item: string) => ({
    ...
    const fetchEquipmentSituation1 = async () => {
      const page_Id = currentPage5.value;
      const page_Size = pageSize5.value;
      const deviceModel = selectedType.value;
      const deviceMac = selectedUID.value;
      const deviceSn = selectedChannel.value;
      const country = selectedsales.value;
    ...
    const fetchEquipmentCountry = async () => {
      try {
        ...
        console.log(response.data.list)
        const uniqueItems = Array.from(new Set(response.data.list)) as string[];
        salesTerritory.value = uniqueItems.map((item: string) => ({
          value: item,
  ...
  const imgLogo = ref('/logoImage.png')

  const imgQuestion = ref('/question_mark.png')

  const imgUpwardTriangle = ref('/upward_triangle.png')

  const imgDownwardTriangle = ref('/downward_triangle.png')

  const equipmentType = ref('WaxJet410')
  ...
  const equipmentOptions = ref<EquipmentTypeOption[]>([]);

  const faultRatePercentage = ref(12)
  const completionRatePercentage = ref(2)
  const utilizationRatePercentage = ref(12)

  const handleEquipmentChange = (value: string) => {
    console.log(value)
    fetchEquipmentPerformance()
    // if (value === 'WJ410') {
    //   faultRatePercentage.value = 12
    //   completionRatePercentage.value = 2
    //   utilizationRatePercentage.value = 12
    // } else if (value === 'WJ400') {
    //   faultRatePercentage.value = 14
    //   completionRatePercentage.value = 6
    //   utilizationRatePercentage.value = 14
    // } else if (value === 'WJ510') {
    //   faultRatePercentage.value = 16
    //   completionRatePercentage.value = 8
    //   utilizationRatePercentage.value = 17
    // } else if (value === 'WJ530') {
    //   faultRatePercentage.value = 9
    //   completionRatePercentage.value = 5
    //   utilizationRatePercentage.value = 18
    // }
  }
  ...
  let initialHeaderMap1 = reactive<HeaderMap1>({
    waxMaterialType: '设备型号',
    deviceMAC: '设备MAC',
    deviceSN: '设备SN',
    activationState: '激活/绑定状态',
    activationTime: '激活/解绑时间',
    activationPeriod: '激活有效期（天）',
    maintenancePeriod: '维保周期',
    softwareRelease: '软件版本',
    softwareReleaseDate: '软件发布日期',
    // 继续其他属性定义
    ...
    fleeGoods: '是否窜货报警',
    reportingTime: '上报时间',
    malfunctionAlarm: '是否故障报警',
    faultType: '故障类型',
    faultReportingTime: '上报时间',
    runningState: '当前设备运行状态'
  });
  ...
  watch(selectedColumns1, (newSelected) => {
    ...
    const mapping = {
      'parent_6_child_0': 'salesOrderNumber',
      'parent_6_child_1': 'salesman',
      'parent_6_child_2': 'customerName',
      'parent_6_child_3': 'orderAddress',
      'parent_6_child_4': 'salesArea',
      'parent_6_child_5': 'outboundOrderNumber',
      'parent_6_child_6': 'outboundTime',
      'parent_7_child_0': 'deviceIP',
      'parent_7_child_1': 'IPArea',
      'parent_7_child_2': 'fleeGoods',
      'parent_7_child_3': 'reportingTime',
      'parent_8_child_0': 'malfunctionAlarm',
      'parent_8_child_1': 'faultType',
      'parent_8_child_2': 'faultReportingTime',
      'parent_8_child_3': 'runningState',
    };
  ...
  const exportToExcel1 = () => {
    ...
    // 导出文件
    writeFile(wb, '设备列表数据.xlsx');
  };

  const handleClick1 = (row: any) => {
    console.log(row.deviceSN)
    navigateTo({
      path: '/equipment/detailInformation',
...
<template>
  <div class="customerDetails">
    ...
    <div class="flex justify-between condition-box">
      <div class="condition flex gap-4">
        <div class="flex justify-center items-center">
          <el-image class="imgLogoSrc" :src="imgLogo" />  
        </div>
        <div class="flex flex-col justify-center">
          <span class="condition-title">工作中</span>
          <span class="condition-number">{{equipmentFailure}}台</span>
        </div>
      </div>
      <div class="condition flex gap-4">
        <div class="flex justify-center items-center">
        ...
        <div class="flex flex-col justify-center">
          <span class="condition-title">异常告警</span>
          <span class="condition-number">{{waxMaterial}}台</span>
        </div>
      </div>
      <div class="condition flex gap-4">
        <div class="flex justify-center items-center">
        ...
        <div class="flex flex-col justify-center">
          <span class="condition-title">待机</span>
          <span class="condition-number">{{equipmentChanneling}}台</span>
        </div>
      </div>
      <div class="condition flex gap-4">
        <div class="flex justify-center items-center">
        ...
        <div class="flex flex-col justify-center">
          <span class="condition-title">脱机</span>
          <span class="condition-number">{{waxPseudo}}台</span>
        </div>
      </div>
    </div>
    <div class="performance-box">
      <div class="flex items-center justify-between" style="margin-bottom: 10px;">
        <div class="flex items-center">
          <span class="performance-title">设备性能：</span>
          <el-select
        ...
        <div class="statisticalTime">统计时间段：{{statisticalTime}}</div>
      </div>
      <div class="flex items-center justify-around">
        <!-- <div style="padding-top: 20px;">
          <div class="flex items-center gap-3">
            <span class="monthly">月度-平均故障率</span>
            <el-image :src="imgQuestion" style="width: 20px; height: 20px;" />
          </div>
          <div>
            <span class="comparison">环比</span>
            <el-image :src="imgUpwardTriangle" style="width: 26px; height: 16px;" />
            <span class="percentage">{{ faultRatePercentage }}%</span>
          </div>
        </div>
        <div class="line-right"></div> -->
        <div style="padding-top: 20px;">
          <div class="flex items-center gap-3">
            <span class="monthly">月度-平均打印完成率</span>
            <el-image :src="imgQuestion" style="width: 20px; height: 20px;" />
          </div>
          <div>
            <span class="comparison">环比</span>
            <el-image :src="imgDownwardTriangle" style="width: 26px; height: 16px;" />
            <span class="percentage">{{ getFormattedAvgCompletion(avgCompletion) }}%</span>
          </div>
        </div>
        <div class="line-right"></div>
        <div style="padding-top: 20px;">
          <div class="flex items-center gap-3">
            <span class="monthly">月度-平均利用率</span>
            <el-image :src="imgQuestion" style="width: 20px; height: 20px;" />
          </div>
          <div>
            <span class="comparison">环比</span>
            <el-image :src="imgUpwardTriangle" style="width: 26px; height: 16px;" />
            <span class="percentage">{{ getFormattedAvgCompletion(avgUtilization) }}%</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="flex items-center justify-between pt-1.5 pb-5">
        <div class="flex flex-col justify-center gap-5">
          ...
          <div class="flex items-center">
            ...
            <div>
              <span class="equipment-type">设备SN</span>
      ...
      >
        ...
        <el-tab-pane label="蜡材情况" name="second">
          <el-table :data="paginatedTableData1" border style="width: 100%">
            <el-table-column type="index" label="序号" width="116" />
            <el-table-column prop="waxMaterialType" label="设备型号" width="115">
              <template #default="{ row, column, $index }">
                <span>{{ row.waxMaterialType !== '' ? row.waxMaterialType : '—' }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="deviceMAC" label="设备MAC" width="210">
              <template #default="{ row, column, $index }">
                <span>{{ row.deviceMAC !== '' ? row.deviceMAC : '—' }}</span>
              </template>
            </el-table-column>
            <el-table-column prop="deviceSN" label="设备SN" min-width="133">
              <template #default="{ row, column, $index }">
                <span>{{ row.deviceSN !== '' ? row.deviceSN : '—' }}</span>
              </template>
            </el-table-column>
