<script lang="ts" setup>
      ...
      try {
        ...
        if (response.code === 0) {
          if (response.data.list.length !== 0) {
            ...
            if ((responseDeviceModel.includes(deviceModel) || responseDeviceMac.includes(deviceMac) || responseName.includes(alertName) || isWithinRange(alertTimeRange.start_at, alertTimeRange.end_at, responseCreateAt)) && (searchClickType.value === false)) {
              cardUid = '';
              deviceModel = '';
              deviceMac = '';
              alertName = '';
              ...
              try {
                ...
                if (response.code === 0) {
                  if (response.data.list.length !== 0) {
                    ...
                    tableData.value = response.data.list.map((item: { create_at: number; name: any; source: any; reason: any; device_model: any; device_sn: any; device_mac: any; country: string; ip: any; ip_location: any; distributor: any; customer: any; }) => ({
  ...
  const allOptions = computed(() => {
    ...
    const { getAlarmRecord, getEquipmentType, getGoodsEscaping } = useRegisterApi();
  ...
  const mapCountryToChinese = (countryCode: string) => {
    ...
    const highIncidenceOfEscape = ref('')
    const highIncidenceAndIntrusion = ref('')
    const gaoFaTiaoHuo = ref('')
    ...
    const fetchGoodsEscaping = async () => {
      try {
        const response = await getGoodsEscaping();
        console.log('User account details fetched successfully:', response);
        console.log(response.data)

        if (response.code === 0) {
          if (response.data.high_frequency_transfer !== null) {
            highIncidenceOfEscape.value = response.data.high_frequency_transfer.dst_addr
            highIncidenceAndIntrusion.value = response.data.high_frequency_transfer.src_addr
            gaoFaTiaoHuo.value = response.data.high_frequency_transfer.distributor
          }

          if (response.data.device_trend !== null) {
            trendAnalysis1.value.num = []
            trendAnalysis1.value.date = []

            response.data.device_trend.map((item: { num: any; date: any; }) => {
          ...
          if (response.data.card_trend !== null) {
            trendAnalysis2.value.num = []
            trendAnalysis2.value.date = []

            response.data.card_trend.map((item: { num: any; date: any; }) => {
  ...
  import ChartComponentHtr from '~/components/ChartComponentHtr.vue';
...
<template>
  <div class="customerDetails">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>设备及蜡材窜货告警</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="flex justify-between items-center" style="margin-top: 20px;">
      <div class="zxt flex items-center" style="width: 60%;">
        <ChartComponentHtr :trendAnalysis1="trendAnalysis1" :trendAnalysis2="trendAnalysis2" />
      </div>
      <div class="zxt-content flex justify-center items-center">
        <div>
          <div class="zxt-title">当月高频窜货行为-榜单</div>
          <div>
            <div class="gfcc" style="margin-bottom: 20px;">高发窜出区：{{highIncidenceOfEscape}}</div>
            <div class="gfcc" style="margin-bottom: 20px;">高发窜入区：{{highIncidenceAndIntrusion}}</div>
            <div class="gfcc">高发窜货经销商：{{gaoFaTiaoHuo}}</div>
          </div>
        </div>
      </div>
    </div>
    <div class="customerDetails-box">窜货记录</div>
