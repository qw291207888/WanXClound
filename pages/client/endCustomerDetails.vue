<script lang="ts" setup>
  ...
  const filters = ref<Filters>({
    ...
    fault_alert: malfunctionAlarmFilter.value,
    transfer_alert: alarmOfGoodsFleeingFilter.value
  });
  ...
  const dynamicColumns = ref([
    { prop: 'distributor', label: '购入渠道', width: '83' },
    { prop: 'activation_status', label: '设备激活/解绑状态', width: '260', filterable: true, filterOptions: [
        { label: '全部', value: 'all' },
        { label: '已激活', value: '已激活' },
        { label: '未激活', value: '未激活' }
      ]},
    ...
    {
      prop: 'device_expire_at',
      label: '设备激活有效期',
      width: '128'
    },
    { prop: 'ip', label: '设备IP地址', width: '128' },
    { prop: 'ip_location', label: 'IP所在区域', width: '126' },
    { prop: 'device_status', label: '设备运行状态', width: '116' },
    { prop: 'fault_alert', label: '是否故障报警', width: '210', filterable: true, filterOptions: [
        { label: '全部', value: 'all' },
        { label: '是', value: '是' },
        { label: '否', value: '否' }
      ]},
    { prop: 'transfer_alert', label: '是否设备窜货报警', width: '240', filterable: true, filterOptions: [
        { label: '全部', value: 'all' },
        { label: '是', value: '是' },
        { label: '否', value: '否' }
      ]},
  ...
  const conversionTimeFormat = (time: number) => {
     const conversionTime = moment.unix(time).format('YYYY-MM-DD HH:mm:ss')
  ...
  // const mapActivationStatus = (status: number): string => {
  //     return status === 0 ? '未激活' : '已激活';
  //   };

  //   const renderCellContent = (prop: string, value: any): string => {
  //     if (prop === 'activation_status') {
  //       return mapActivationStatus(value);
  //     }
  ...
  const dynamicColumns1 = ref([
    ...
    },
    {
      ...
      columns: [
        ...
        { prop: 'falseAlarm', label: '是否设备窜货报警', width: '240', sortable: false , filterable: true, filterOptions: [
              { label: '全部', value: 'all' },
              { label: '是', value: '是' },
              { label: '否', value: '否' }
           ]},
  ...
  const allOptions = computed(() => {
    ...
    const { getCustomerDetails, getEquipmentSituation, getEquipmentType, getCountriesList, getMaterialCondition } = useRegisterApi();
    ...
    const customerDistributors = ref('')
    const historicalActivationNum = ref('0')
    ...
    const fetchCustomerDetails = async (pageId: number | null) => {
      ...
      try {
        const response = await getCustomerDetails(pageId);
        ...
        customerAddress.value = response.data.address;
        customerCountry.value = response.data.country;
        customerDistributors.value = response.data.distributors.join('/');
        historicalActivationNum.value = response.data.historical_activation_num;
    ...
    const fetchEquipmentSituation = async (pageId: number | null) => {
      ...
      const customerMode = selectedModel.value;
      const customerMAC = selectedMAC.value;
      const customerTerritory = selectedTerritory.value;
      const customerSN = selectedSN.value;
    
      try {
        const response = await getEquipmentSituation(page_Id, page_Size, pageId, customerMode, customerMAC, customerTerritory, customerSN);
        console.log('User account details fetched successfully:', response);
    ...
    const fetchEquipmentSituation1 = async (pageId: number | null) => {
      ...
      try {
        const response = await getMaterialCondition(page_Id, page_Size, pageId, deviceModel1, deviceMac1, customerTerritory1, customerSN1);
        console.log('User account details fetched successfully:', response);
        console.log(response.data.list);
        if (response.data.list === null) {
          tableData1.value = []
...
uid: any;
model: any; country: any; distributor: any; activation_status: number; activation_at: number; transfer_alert: any; illegal_alert: any; alert_at: number; 
}; }) => ({
          waxMaterialType: item.card_detail.model,
          waxUID: item.card_detail.uid,
          salesTerritory: item.card_detail.country,
          purchaseChannel: item.card_detail.distributor,
    ...
    onMounted(async () => {
      ...
      //     customerDistributors.value = response.data.distributors.join('/')
      //     historicalActivationNum.value = response.data.historical_activation_num
      //     alertNumStats.value = response.data.alert_num_stats.total
  ...
  const waxMaterialType = ref<EquipmentTypeOption[]>([]);
  ...
  const salesTerritory = ref<EquipmentTypeOption[]>([]);
  ...
  const resetFilters = () => {
    ...
    fetchEquipmentSituation(pageId)
    // filters.value.activation_status = 'all'
    // filters.value.fault_alert = 'all'
    // filters.value.transfer_alert = 'all'
  };
  ...
  let initialHeaderMap = reactive<HeaderMap>({
    ...
    country: '销售区域',
    distributor: '购入渠道',
    activation_status: '设备激活/解绑状态',
    activation_at: '激活/解绑时间',
    device_expire_at: '设备激活有效期',
    ip: '设备IP地址',
    ip_location: 'IP所在区域',
    device_status: '设备运行状态',
    fault_alert: '是否故障报警',
    transfer_alert: '是否设备窜货报警',
    alert_at: '最近一次触发报警'
  });
  ...
  //   // 合并 transformedArray 和 newHeaderMap
  //   const mergedHeaderMap: HeaderMap = { ...newHeaderMap };
  ...
  // console.log(initialHeaderMap1)

  // // 可见的表头（用户可以动态修改这个数组）
  // const visibleHeaders1 = ref<string[]>(Object.keys(initialHeaderMap1));

  // console.log(visibleHeaders1)
  ...
  //   // 将数据转换成带有动态表头的数据
  //   const dataWithDynamicHeaders1 = tableData1.value.map(item => {
  //     const newItem1: { [key: string]: any } = {};
  //     keys1.forEach(key => {
  //       newItem1[dynamicHeaderMap1[key]] = (item as any)[key];
  //     });
  //     return newItem1;
  //   });
  
  //   // 在数据前添加表头
  //   const excelData1 = [headers1, ...dataWithDynamicHeaders1.map(item => Object.values(item))];

  //   console.log(excelData1)
  
  //   // 创建工作表
  //   const ws = utils.aoa_to_sheet(excelData1);
  //   const wb = utils.book_new();
  //   utils.book_append_sheet(wb, ws, 'Sheet1');
...
<template>
  <div class="customerDetails">
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>终端客户详情</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="customer-content">
      <h1 class="customer-title">{{ customerName }}</h1>
      <div class="customer-first">
        <span class="customer-address">邮箱地址：</span>
        <span class="customer-address-content">{{ customerEmail }}</span>
      </div>
      <div class="customer-first">
        <span class="customer-address">地址：</span>
        <span class="customer-address-content">{{ customerAddress }}</span>
      </div>
      <div class="customer-first">
        <span class="customer-address">所属销售区：</span>
        <span class="customer-address-content">{{ customerCountry }}</span>
      </div>
      <div>
        <span class="customer-address">购入渠道：</span>
        <span class="customer-address-content">{{ customerDistributors }}</span>
      </div>
    </div>
    <div class="flex justify-between items-center card-code">
      <div class="card-first flex flex-col items-center justify-center">
        ...
        <span class="flex justify-center">{{ historicalActivationNum }}</span>
      </div>
          ...
          <el-table :data="paginatedTableData" border style="width: 100%">
            ...
            <template v-for="column in dynamicColumns" :key="column.prop">
              ...
              >
                ...
                <template v-slot="scope" v-if="column.prop === 'activation_at'">
                  {{ conversionTimeFormat(scope.row[column.prop]) }}
                </template>
